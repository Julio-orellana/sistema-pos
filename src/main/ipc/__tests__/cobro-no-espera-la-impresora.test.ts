...
describe('Cobrar confirma la venta sin esperar a la impresora', () => {
  ...
  it('CUANDO LA IMPRESORA CONTESTA, la ventana que cobró recibe el aviso con su recibo', async () => {
    ...
    const aviso: ImpresionDeReciboTerminadaIpc = {
      ...
      numeroRecibo: 1,
      impreso: true,
      // AJUSTADO AL UNIR develop CON LA SPEC 001 (2026-09-18): antes decía
      // «Recibo enviado a la impresora.»; el servicio ahora nombra las dos
      // copias (CLAUDE.md §4.65). El aviso a la ventana es el mismo.
