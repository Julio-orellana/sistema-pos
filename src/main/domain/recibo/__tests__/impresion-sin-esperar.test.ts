...
describe('Al cobrar, el recibo NO espera a la impresora', () => {
  ...
  it('cuando la impresora contesta, la impresión termina SOLA y marca el recibo impreso', async () => {
    ...
    /*
      AJUSTADO AL UNIR develop CON LA SPEC 001 (2026-09-18): esta prueba nació
      esperando «Recibo enviado a la impresora.», que era lo que el servicio
      copiaba de la impresora. Desde la spec 001 el servicio dice qué copias
      salieron (CLAUDE.md §4.65). Lo que esta prueba fija —que la impresión
      termina sola y marca el recibo— no cambió.
    */
    await expect(emision.impresion).resolves.toEqual({
      impreso: true,
      mensaje: 'Recibo enviado a la impresora: copia del cliente y copia de la tienda.',
    });
