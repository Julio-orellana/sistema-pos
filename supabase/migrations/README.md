...
| `0033_anulaciones_de_venta.sql` | ~~**No — aplicada en `pos-pruebas-descartable` el 2026-09-17 (versión `20260917182250`, md5 `167821022f76e54528355f888f8fecb7`, igual al archivo sin el salto final), y NO en el real.**~~ **Sí — aplicada en el real el 2026-09-17 a las 21:51:45 UTC (versión `20260917215145`) por Julio, desde otra sesión; en `pos-pruebas-descartable` desde las 18:22 UTC del mismo día (versión `20260917182250`, md5 `167821022f76e54528355f888f8fecb7`, igual al archivo sin el salto final).** Corregido el 2026-09-18 leyendo `list_migrations` de los dos proyectos: esta fila seguía diciendo «NO en el real» y CLAUDE.md §4.4 ya lo había corregido. Antes se ensayó en un Postgres 17 LOCAL con las migraciones de esta carpeta (CLAUDE.md §4.53). Evidencia del catálogo en CLAUDE.md §4.4 y §4.56. |
| `0035_sincronizar_anulacion_de_venta.sql` | ~~**No — aplicada en `pos-pruebas-descartable` el 2026-09-17 (versión `20260917182506`, md5 `7e230bd8b5ac2f27efeffbaaef7b6e1f`), justo después de la `0033`, y NO en el real.**~~ **Sí — aplicada en el real el 2026-09-17 a las 21:53:43 UTC (versión `20260917215343`), por Julio, desde otra sesión; en `pos-pruebas-descartable` desde las 18:25 UTC (versión `20260917182506`, md5 `7e230bd8b5ac2f27efeffbaaef7b6e1f`).** Corregido el 2026-09-18, leído del catálogo. Las tres puertas que tienen que cerrarse dieron `42501`. Ver CLAUDE.md §4.4 y §4.56. |
| `0038_anulacion_solo_presencial.sql` | ~~**No — aplicada en `pos-pruebas-descartable` el 2026-09-17 (versión `20260917182736`, md5 `db79f130845f2da8034d124d24163993`), tercera de la ronda, y NO en el real.**~~ **Sí — aplicada en el real el 2026-09-17 a las 21:54:58 UTC (versión `20260917215458`), por Julio, desde otra sesión; su registro se corrigió ese día para que guarde el `;` final (CLAUDE.md §4.4). En `pos-pruebas-descartable` desde las 18:27 UTC (versión `20260917182736`, md5 `db79f130845f2da8034d124d24163993`).** Corregido el 2026-09-18, leído del catálogo. Espejo de la `038` local: CHECK `anulaciones_de_venta_solo_presencial`, `convalidated = true`, que convive con el de la columna. Ver CLAUDE.md §4.54 y §4.56. |
| `0039_productos_precio_mayorista.sql` | ~~**No — escrita el 2026-09-18 y NO APLICADA EN NINGÚN PROYECTO.**~~ **Aplicada en `pos-pruebas-descartable` el 2026-09-18 a las 19:03 UTC (versión `20260918190337`, md5 `9cf8d81d15713cd2f1567d9fac40ef43`, igual al del archivo sin el salto final, con su `;`), con la aprobación de Julio. ~~NO en `pos-jimmy-cano`: ahí solo con un pedido aparte, el día que se instale la versión con la 039.~~** **CORREGIDO EL 2026-09-19, leído del catálogo del real a las 14:21 UTC: también está en `pos-jimmy-cano`, versión `20260919041539`, aplicada fuera de la sesión que escribe esta fila; la evidencia de esa aplicación no está en este repositorio.** Antes de aplicarla, `productos_precio_mayorista_no_negativo` (`>= 0`) pasó a `productos_precio_mayorista_positivo` (`> 0`) por decisión de Julio. Espejo de la `039` local (spec 002, precio mayorista por cantidad mínima): `productos.precio_mayorista` `NUMERIC(14,2)` y `productos.cantidad_minima_mayorista` `NUMERIC(14,3)`, nulables, con `productos_mayorista_completo` y `productos_mayorista_menor_que_lista` con el MISMO nombre que en la local. No toca ninguna función y la versión de contrato no sube. **Es una migración PAREJA, como la `0028`**: una terminal con la `039` contra una nube sin la `0039` detiene su cola en el primer lote de productos o de venta (medido en un Postgres 17 local: `HTTP 400 … CONTRATO: el payload de public.productos trae columnas que la tabla no tiene: cantidad_minima_mayorista, precio_mayorista`), y al revés también. Espera la decisión 1 de la spec (§4.3) y la aprobación de Julio, proyecto por proyecto. Leído del catálogo el 2026-09-18 a las 17:58 UTC: los dos proyectos tienen 29 migraciones, la última la `0038`, y ninguna columna mayorista. Ver CLAUDE.md §4.66. |
| `0040_anulacion_autorizacion_remota.sql` | **Aplicada en `pos-pruebas-descartable` el 2026-09-19 a las 12:06:22 UTC (versión `20260919120622`, md5 `46ace139c0ef2d0243fb41ef51cc8565`, igual al del archivo sin el salto final, con su `;`), con la aprobación de Julio. NO en `pos-jimmy-cano`: ahí solo con un pedido aparte, ANTES de instalar en la tienda la versión que trae la `040`.** Espejo de la `040` local (spec 003): quita `anulaciones_de_venta_solo_presencial`, que puso la `0038`, **a pedido explícito del cliente**; la `0038` no se edita. Deja el CHECK de la columna. **Es una relajación pura y NO es pareja:** no cambia la forma del payload ni la versión de contrato (huellas de columnas y funciones iguales antes y después, contrato en 1, `verify:nube` sin diferencias), así que no detiene la cola de ninguna versión y puede ir antes que la versión nueva. Al revés sí traba: una terminal con la `040` contra una nube sin la `0040` recibe `23514` en su primera anulación a distancia (leído en el código, no medido). Ver CLAUDE.md §4.70. |

**ESTADO AL 2026-09-19, leído del catálogo a las 14:21 UTC:** `pos-jimmy-cano`
tiene **30** migraciones, la última la `0039` (aplicada ese día a las 04:15:39
UTC, fuera de esta sesión); `pos-pruebas-descartable` tiene **31**, la última la
`0040`. Lo que sigue se conserva como constancia de los estados anteriores.

**CORREGIDO EL 2026-09-18, leyendo `list_migrations` de los dos proyectos: los
dos tienen las mismas 29**, la última la `0038`. Al real, la `0033`, la `0035` y
la `0038` se las aplicó Julio desde otra sesión el 2026-09-17 entre las 21:51 y
las 21:54 UTC (CLAUDE.md §4.4). **Ninguno de los dos tiene la `0039`** ~~.~~ **(corregido el mismo día a las 19:03 UTC: el descartable sí, 30 migraciones; el real sigue en 29).** Lo que
sigue quedó escrito antes de saberlo, y se conserva como constancia:
