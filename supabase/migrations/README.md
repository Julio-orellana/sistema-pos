...
| `0039_productos_precio_mayorista.sql` | **No — escrita el 2026-09-18 y NO APLICADA EN NINGÚN PROYECTO.** Espejo de la `039` local (spec 002, precio mayorista por cantidad mínima): `productos.precio_mayorista` `NUMERIC(14,2)` y `productos.cantidad_minima_mayorista` `NUMERIC(14,3)`, nulables, con `productos_mayorista_completo` y `productos_mayorista_menor_que_lista` con el MISMO nombre que en la local. No toca ninguna función y la versión de contrato no sube. **Es una migración PAREJA, como la `0028`**: una terminal con la `039` contra una nube sin la `0039` detiene su cola en el primer lote de productos o de venta (medido en un Postgres 17 local: `HTTP 400 … CONTRATO: el payload de public.productos trae columnas que la tabla no tiene: cantidad_minima_mayorista, precio_mayorista`), y al revés también. Espera la decisión 1 de la spec (§4.3) y la aprobación de Julio, proyecto por proyecto. Leído del catálogo el 2026-09-18 a las 17:58 UTC: los dos proyectos tienen 29 migraciones, la última la `0038`, y ninguna columna mayorista. Ver CLAUDE.md §4.66. |
...
las 21:54 UTC (CLAUDE.md §4.4). **Ninguno de los dos tiene la `0039`.** Lo que
