...
| 39 | **`escpos.ts` exporta una segunda constante, `COLUMNAS_TERMICA = 48`, que nadie usa.** | Encontrado el 2026-09-17 al confirmar el ancho del RPT004: ningún archivo de `src` ni de `scripts` la importa, y el ancho real del recibo sale de `COLUMNAS_80MM`. Hoy las dos dicen 48, así que no cambia nada; pero son dos lugares con el mismo número, y el día que alguien cambie uno el otro va a mentir. No se tocó: era un commit de documentación. | Abierto — quitarla o hacerla la única fuente, a decidir |
